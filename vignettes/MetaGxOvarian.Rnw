...
The package is publicly available and can be installed from Bioconductor into R version 3.4.1 or higher.

To install the MetaGxOvarian package from Bioconductor:
<<installAndLoadPackagesFromBioconductor,eval=FALSE,results='hide',message=FALSE, warning = F>>=
...
<<LoadPackages,eval=TRUE,comment="##",results='hide',message=FALSE, warning = F>>=
...
This will load 26 expression datasets. Users can modify the parameters of the loadOvarianEsets function to adjust the datasets based on a variety of criteria. Some of which include:
